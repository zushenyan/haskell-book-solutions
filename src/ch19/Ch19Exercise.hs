...
import Web.Scotty (ScottyM, get, html, param, scotty, text)
...
checkExists :: R.Connection -> BC.ByteString -> IO (Either R.Reply Bool)
checkExists conn shortURI = R.runRedis conn $ R.exists shortURI

linkShorty :: String -> String
linkShorty s = concat ["<a href=\"", s, "\">Copy and paste your short URL<\a>"]
  ...
  get "/" $ do
    ...
    case parsedUri of
      Just _ -> do
        ...
        existence <- liftIO $ checkExists rConn shorty
        case existence of
          Left reply -> text (TL.pack $ show reply)
          Right b ->
            if b
              then text (TL.pack "collision happened!")
              else do
