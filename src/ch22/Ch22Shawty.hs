...
import Web.Scotty (ActionM, ScottyM, get, html, param, scotty, text)
...
getHome :: ReaderT R.Connection ScottyM ()
getHome = do
  rConn <- ask
  lift $
    get "/" $ do
...
getShort :: ReaderT R.Connection ScottyM ()
getShort = do
  rConn <- ask
  lift $
    get "/:short" $ do
      short <- param "short"
      uri <- lift $ runReaderT (getURI short) rConn
      case uri of
