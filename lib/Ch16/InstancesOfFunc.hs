...
-- fc :: (Functor f, Eq (f c)) => (a -> b) -> (b -> c) -> f a -> Bool
-- fc :: (Functor f) => (a -> b) -> (b -> c) -> f a -> Bool
-- fc f g x = fmap g (fmap f x) == fmap (g . f) x

-- fc :: Fun Float Char -> Fun Char (Maybe Int) -> Maybe Float -> Bool
...
test1 = do
  ...
  quickCheck fc
  where
    fc :: Fun Int Char -> Fun Char Float -> T1 -> Bool
    fc (Fun _ f) (Fun _ g) x = fmap g (fmap f x) == fmap (g . f) x
...
test2 = do
  ...
  quickCheck fc
  where
    fc :: Fun Int Char -> Fun Char Float -> T2 -> Bool
    fc (Fun _ f) (Fun _ g) x = fmap g (fmap f x) == fmap (g . f) x
...
test3 = do
  ...
  quickCheck fc
  where
    fc :: Fun Int Char -> Fun Char Float -> T3 -> Bool
    fc (Fun _ f) (Fun _ g) x = fmap g (fmap f x) == fmap (g . f) x
...
test4 = do
  ...
  quickCheck fc
  where
    fc :: Fun Int Char -> Fun Char Float -> T4 -> Bool
    fc (Fun _ f) (Fun _ g) x = fmap g (fmap f x) == fmap (g . f) x
...
test5 = do
  ...
  quickCheck fc
  where
    fc :: Fun Int Char -> Fun Char Float -> T5 -> Bool
    fc (Fun _ f) (Fun _ g) x = fmap g (fmap f x) == fmap (g . f) x
...
test6 = do
  ...
  quickCheck fc
  where
    fc :: Fun Int Char -> Fun Char Float -> T6 -> Bool
    fc (Fun _ f) (Fun _ g) x = fmap g (fmap f x) == fmap (g . f) x
...
test7 = do
  ...
  quickCheck fc
  where
    fc :: Fun Int Char -> Fun Char Float -> T7 -> Bool
    fc (Fun _ f) (Fun _ g) x = fmap g (fmap f x) == fmap (g . f) x
